...
export default function Projects() {
  ...
  return (
    <Stack gap="xl" mb="xl" pb="xl" style={{ borderBottom: '1px solid var(--mantine-color-gray-3)' }}>
      <Group gap="sm">
        <IconFolder size={32} />
        <Title order={2} id="projects-heading">Projects</Title>
      </Group>
          ...
          >
            ...
            <Stack gap="sm" p="lg">
              <Group justify="space-between" align="flex-start">
                ...
                <Badge variant="light" color={project.status === "In Progress" ? "blue" : "gray"} aria-label={`Project status: ${project.status}`}>
                  {project.status}
                </Badge>
              </Group>
              ...
              <Group gap="xs" mt="xs" role="list" aria-label="Technologies used">
                {(project.tags || project.technologies || []).slice(0, 4).map((tech, i) => (
                  <Badge key={i} variant="filled" size="sm" role="listitem">{tech}</Badge>
                ))}
